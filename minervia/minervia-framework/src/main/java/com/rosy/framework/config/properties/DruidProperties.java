...
public class DruidProperties {
    ...
    @Value("${spring.datasource.druid.initialSize:10}")
    ...
    @Value("${spring.datasource.druid.connectTimeout:5000}")
    private int connectTimeout;

    @Value("${spring.datasource.druid.socketTimeout:5000}")
    ...
    @Value("${spring.datasource.druid.maxEvictableIdleTimeMillis:1800000}")
     ...
     */
    public DruidDataSource configureDataSource(DruidDataSource datasource) {
        try {
            // 配置初始化大小、最小、最大
            ...
            // 配置用来检测连接是否有效的sql，要求是一个查询语句，常用select 'x'。如果validationQuery为null，testOnBorrow、testOnReturn、testWhileIdle都不会起作用。
            ...
            // 异常处理
            throw new RuntimeException("配置 Druid 数据源时发生错误: " + e.getMessage(), e);
        }
        return datasource;
    }
}
